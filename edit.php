...
$sql2 = $conn->query("SELECT low_class.modell
     ...
     if (count($_POST) > 0) {
            ...
            while ($row2 = $sql2->fetch_assoc()){
                ...
                if ($class == "low_class") {
                    $sql = "UPDATE low_class SET price_per_day = $price_per_day, fk_availability_id = $availability WHERE low_class.modell = '$cars_modell'";
                    $edit = $conn->query($sql);
                } elseif ($class == "mid_class") {
                    $sql = "UPDATE mid_class SET price_per_day = $price_per_day, fk_availability_id = $availability WHERE mid_class.modell = '$cars_modell'";
                    $edit = $conn->query($sql);
                } else {
                    $sql = "UPDATE luxury_class SET price_per_day = $price_per_day, fk_availability_id = $availability WHERE luxury_class.modell = '$cars_modell'";
                    $edit = $conn->query($sql);
                }
            }
...
?>
    ...
    <form class = "main_form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" enctype="multipart/form-data">
        ...
        <button class="btn btn-primary mt-3 mb-5" type="submit" class="registerbtn">Edit</button>
    </form>
    </div>
